...
func (nd *LitNode) DualFundingReqHandler(msg lnutil.DualFundingReqMsg) {
	...
	if !ok {
		...
		log.Errorf("Auto declining dual fund request. We don't handle coin type %d\n", msg.CoinType)
	...
	nd.InProgDual.mtx.Lock()
	if nd.InProgDual.PeerIdx != 0 {
		log.Errorf("DualFundingReqHandler already have a pending request. Declining.")
	...
	if msg.TheirAmount > spendable-50000 {
		log.Errorf("Auto declining dual fund request. Requested %d, but we only have %d for funding\n", msg.TheirAmount, spendable-50000)
...
func (nd *LitNode) DualFundChanDescHandler(msg lnutil.ChanDescMsg) {
	...
	log.Info("DualFundChanDescHandler\n")

	wal, ok := nd.SubWallet[msg.CoinType]
	if !ok {
		log.Errorf("DualFundChanDescHandler err no wallet for type %d", msg.CoinType)
		return
	}
	...
	cIdx, err := nd.NextChannelIdx()
	if err != nil {
		log.Errorf("DualFundChanDescHandler err %s", err.Error())
		return
	}
	...
	if err != nil {
		log.Errorf("DualFundChanDescHandler GetQchan err %s", err.Error())
		return
	}
	...
	if err != nil {
		log.Errorf("DualFundChanDescHandler SignState err %s", err.Error())
		return
	}

	log.Info("Acking channel...\n")

	fundingTx, err := nd.BuildDualFundingTransaction()
	if err != nil {
		log.Errorf("DualFundChanDescHandler BuildDualFundingTransaction err %s", err.Error())
		return
	}
...
func (nd *LitNode) DualFundChanAckHandler(msg lnutil.DualFundingChanAckMsg, peer *RemotePeer) {
	...
	log.Info("Registering refund address in the wallet\n")
