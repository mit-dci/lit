...
func (nd *LitNode) OfferHTLC(qc *Qchan, amt uint32, RHash [32]byte, locktime uint32, data [32]byte) error {
	...
	Log.Infof("OfferHTLC: Sending HashSig")
	...
	Log.Info("OfferHTLC: Done: sent HashSig")

	Log.Info("got pre CTS...")
	qc.ChanMtx.Unlock()
...
func (nd *LitNode) SendHashSig(q *Qchan) error {
	...
	Log.Infof("Sending HashSig with %d HTLC sigs", len(HTLCSigs))
...
func (nd *LitNode) ClearHTLC(qc *Qchan, R [16]byte, HTLCIdx uint32, data [32]byte) error {
	...
	Log.Info("ClearHTLC: Sending PreimageSig")
...
func (nd *LitNode) SendPreimageSig(q *Qchan, Idx uint32) error {
	...
	Log.Infof("Sending PreimageSig with %d HTLC sigs", len(HTLCSigs))
...
func (nd *LitNode) PreimageSigHandler(msg lnutil.PreimageSigMsg, qc *Qchan) error {
	Log.Infof("Got PreimageSig: %v", msg)
	...
	if !timeout {
		...
		go func() {
			...
			if err != nil {
				Log.Errorf("error claiming HTLCs: %s", err.Error())
			}

			for _, id := range txids {
				Log.Infof("claimed HTLC with txid: %x", id)
			}
		}()
	}
...
func (nd *LitNode) SetHTLCClearedOnChain(q *Qchan, h HTLC) error {
	...
	if err != nil {
		Log.Errorf("Error reloading qchan state: %s", err.Error())
		q.ChanMtx.Unlock()
		return err
	}
	...
	if err != nil {
		Log.Errorf("Error saving qchan state: %s", err.Error())
		q.ChanMtx.Unlock()
		return err
	}
	q.ChanMtx.Unlock()
	...
	for i, h := range htlcs {
		...
		if h.Incoming && !h.Cleared {
			...
			if q.CloseData.Closed {
				...
				if err != nil {
					Log.Errorf("Error claiming HTLC: %s", err.Error())
					return nil, err
				}
				...
				Log.Infof("Cleaing HTLC from channel [%d] idx [%d]\n", q.Idx(), h.Idx)
...
func (nd *LitNode) ClaimHTLCTimeouts(coinType uint32, height int32) ([][32]byte, error) {
	...
	if len(htlcs) > 0 {
		Log.Infof("Found [%d] HTLC Outpoints that have timed out\n", len(htlcs))
		for i, h := range htlcs {
			if !h.Incoming { // only for timed out HTLCs!
				...
				if q.CloseData.Closed {
					...
					Log.Infof("Timing out HTLC from channel [%d] idx [%d]\n", q.Idx(), h.Idx)
			...
			for _, h := range q.State.HTLCs {
				if !h.Incoming && !h.Cleared {
					if height >= int32(h.Locktime) {
						...
						Log.Infof("Ignoring HTLC in chan [%d] idx [%d] - expires at block [%d] (now: %d)", q.Idx(), h.Idx, h.Locktime, height)
					}
				}
			}
		}
...
func (nd *LitNode) ClaimHTLCOnChain(q *Qchan, h HTLC) (*wire.MsgTx, error) {
	...
	Log.Debug("Claiming HTLC on-chain. TX:")
	lnutil.PrintTx(tx)
	...
	if mine {
		...
		if !bytes.Equal(genSH[:], tx.TxOut[0].PkScript[2:34]) {
			Log.Warnf("got different observed and generated SH scripts.\n")
			Log.Warnf("in %s:%d, see %x\n", tx.TxHash().String(), 0, tx.TxOut[0].PkScript)
			Log.Warnf("generated %x \n", genSH)
			Log.Warnf("revokable pub %x\ntimeout pub %x\n", revokePub, timeoutPub)
			return tx, nil
		}
