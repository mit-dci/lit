...
func (s *SPVCon) GetListOfNodes() ([]string, error) {
	...
	Log.Infof("Attempting to retrieve peers to connect to based on DNS Seed\n")
	...
	for _, seed := range s.Param.DNSSeeds {
		...
		if err != nil {
			Log.Infof("Have difficulty trying to connect to %s. Going to the next seed", seed)
			continue
		}
...
func (s *SPVCon) DialNode(listOfNodes []string) (net.Conn, error) {
	...
	for i, ip := range listOfNodes {
		...
		if err != nil {
			Log.Infof("parse error for node (skipped): %s", err)
			continue
		}
...
func (s *SPVCon) Handshake(listOfNodes []string) error {
	...
	Log.Infof("remote reports version %x (dec %d)\n",
...
func (s *SPVCon) Connect(remoteNode string) error {
	...
	for len(listOfNodes) != 0 {
		...
		if err != nil {
			Log.Error(err)
			Log.Infof("Couldn't dial node %s, Moving on", listOfNodes[0])
		...
		if err != nil {
			...
			Log.Infof("Handshake with %s failed. Moving on. Error: %s", listOfNodes[0], err.Error())
