...
func (s *SPVCon) GimmeFilter() (*bloom.Filter, error) {
	...
	for a160 := range s.TrackingAdrs { // add 20-byte pubkeyhash
		//		Log.Infof("adding address hash %x\n", a160)
		f.Add(a160[:])
	}
	...
	Log.Infof("made %d element filter\n", filterElements)
	return f, nil
}
...
func (s *SPVCon) MatchTx(tx *wire.MsgTx) bool {
	...
	for i, out := range tx.TxOut {
		...
		//		Log.Infof("got output key %x ", adr20)
...
func (s *SPVCon) IngestMerkleBlock(m *wire.MsgMerkleBlock) {
	...
	case hah = <-s.blockQueue: // pop height off mblock queue
		break
	default:
		Log.Errorf("Unrequested merkle block")
		return
	}
	...
	if !hah.blockhash.IsEqual(&newMerkBlockSha) {
		...
		Log.Errorf("has %d hashes %d txs flags: %x",
...
func (s *SPVCon) IngestHeaders(m *wire.MsgHeaders) (bool, error) {
	...
	if gotNum > 0 {
		...
		Log.Infof("got 0 headers, we're probably synced up")
		return false, nil
	}
...
func (s *SPVCon) AskForHeaders() error {
	...
	if err != nil {
		Log.Errorf("AskForHeaders GetHeaderAtHeight error\n")
		return err
	}
...
func (s *SPVCon) AskForBlocks() error {
	...
	if s.syncHeight == headerTip {
		...
		Log.Infof("no blocks to request, entering wait state\n")
	...
	Log.Errorf("will request blocks %d to %d\n", s.syncHeight+1, headerTip)
	...
	for reqHeight < headerTip {
		...
		if err != nil {
			Log.Errorf("header deserialize error!\n")
			return err
		}
